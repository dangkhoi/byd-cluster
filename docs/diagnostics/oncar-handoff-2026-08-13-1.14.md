...
3. **Chốt value↔menu** (độc lập app, để set default đúng) — sweep bằng navopen + chụp:
   ```bash
   for v in 0 1 2 3; do
     ...
     sleep 2; adb -s $VEH shell "fission_screencap -d 0 -p /data/local/tmp/s$v.png"; adb -s $VEH pull /data/local/tmp/s$v.png ./mode-$v.png
   done
   ```
   → ghi bảng value→(Đơn giản/Nhỏ/Toàn/OFF). Báo lại con số = "Đơn giản (Giữa+ETA)" để set default `Prefs.NAV_SCREEN_SIMPLE`.
