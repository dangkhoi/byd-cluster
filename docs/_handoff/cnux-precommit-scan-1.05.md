...
1. **`docs/_handoff/session-2026-08-06-pm2-firstlaunch-fixes-and-hud-enable.md:4,41,59`** — personal-hotspot vehicle IP `172.20.10.<x>:5555` ×3. Redact → `<vehicle-ip>`.
2. **`docs/_handoff/hud-cluster-injection-findings-2026-08-10.md:492`** — `172.20.10.<x>` ("Confirmed on-car"). Redact → `<vehicle-ip>`.
3. **`scripts/vehicle/hud-cluster-probe.sh:19`** — `172.20.10.<x>:5555` in a usage-comment example. Redact → `<ip:port>` (the script body already uses `<serial|ip:port>` placeholders elsewhere).
4. **`scripts/re/tests/test_expand_candidate_coverage.py:319`** — the **real** vehicle IP `10.49.0.<x>` used as a scrubber-test fixture input. Recommend replacing with a synthetic RFC1918 (e.g. `10.0.0.1`) so the real IP is not embedded in a test.
