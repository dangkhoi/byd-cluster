...
object CastPlanner {
    fun plan(intent: CastIntent, snapshot: PlannerSnapshot): PlanResult {
        ...
        val steps = when (intent.kind) {
            ...
            CastIntentKind.APPLY_GEOMETRY -> {
                if (intent.geometry == null || observed.target == null || observed.target.packageName != intent.targetPackage ||
                    intent.expectedTarget != observed.target
                ) {
                ...
                buildList {
                    ...
                    if (intent.geometry.densityDpi != null && observed.protectedResidue == null &&
    ...
    /**
     * Field-proven placement ladder. Rungs are ordered by how much they can destroy, and every rung
     * is a journaled step: prepare the display contract, keep the running session (R1), reparent the
     * task when ActivityStarter silently redirected it (R2), reassert to force composite, fit the
     * task to the real cluster size, and only then — with explicit user opt-in and for a normal app —
     * escalate to the destructive relaunch (R3).
     */
    private fun placementSteps(
        ...
        return buildList {
            add(PlannedStep("allow-resizable", CommandKind.SET_FORCE_RESIZABLE, "freeform placement requires resizable activities"))
