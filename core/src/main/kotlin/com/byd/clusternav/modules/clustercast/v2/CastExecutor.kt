...
class CastExecutor(
    ...
    /**
     * Runs [block] only if no mutation is in flight, otherwise returns null immediately.
     *
     * Anything on a rendering path must use this. Waiting on the lease from the UI is how the screen
     * froze on the vehicle 2026-07-27: a wedged operation held it while the refresh path tried to
     * reconcile, so the screen stopped responding entirely.
     */
    internal fun <T> tryMutationLease(block: () -> T): T? {
        if (!lease.tryLock()) return null
        return try {
            block()
        } finally {
            lease.unlock()
